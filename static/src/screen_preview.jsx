...
function ScreenPreview({ onNext, onBack, state }) {
  ...
  return (
    <div>
      ...
      {/* KPI strip — from real biometricSummary */}
      {preview && (() => {
      ...
      {/* HR chart — empty state when no samples */}
      {preview && preview.heartRateSamples.length === 0 && (
        <div className="card" style={{ marginTop: 20, padding: 24, textAlign: 'center', color: 'var(--ink-3)' }}>
      ...
      {/* HR chart — real samples */}
      {preview && preview.heartRateSamples.length > 0 && (() => {
        const samples = preview.heartRateSamples;
        const xScale = samples.length > 1 ? 1140 / (samples.length - 1) : 1140;
